...
void Uart2TxDataIT(uint8_t* pucTxDataBuff, uint16_t uhTxDataSize)
{
	if(NULL != pucTxDataBuff
	&& NULL != (pucTxDataBuff + uhTxDataSize))
	{

		HAL_UART_Transmit_IT(&tUart2, pucTxDataBuff, uhTxDataSize);
	}

	return;
}
void Uart2RxDataIT(uint8_t* pucRxDataBuff, uint16_t uhRxDataSize)
{
	if(NULL != pucRxDataBuff
	&& NULL != pucRxDataBuff + uhRxDataSize)
	{
		HAL_UART_Receive_IT(&tUart2, pucRxDataBuff, uhRxDataSize);
	}
}

void USART2_IRQHandler(void)
{
	HAL_UART_IRQHandler(&tUart2);
}
