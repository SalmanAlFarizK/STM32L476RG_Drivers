...
void ADC_PC0_DMA_Init(void)
{
	...
	tAdc1.Init.DataAlign = ADC_DATAALIGN_RIGHT;
	tAdc1.Init.NbrOfConversion = 1;
	tAdc1.Init.DMAContinuousRequests = ENABLE;
	tAdc1.Init.EOCSelection = ADC_EOC_SINGLE_CONV;
	...
	tAdc1Dma.Instance = DMA1_Channel1;
	tAdc1Dma.Init.Direction = DMA_PERIPH_TO_MEMORY;
	tAdc1Dma.Init.PeriphInc = DMA_PINC_DISABLE;
	tAdc1Dma.Init.MemInc = DMA_MINC_ENABLE;
	tAdc1Dma.Init.PeriphDataAlignment = DMA_PDATAALIGN_HALFWORD;
	tAdc1Dma.Init.MemDataAlignment = DMA_MDATAALIGN_HALFWORD;
	tAdc1Dma.Init.Mode = DMA_CIRCULAR;
	tAdc1Dma.Init.Priority = DMA_PRIORITY_LOW;

	HAL_DMA_Init(&tAdc1Dma);

	tAdc1.DMA_Handle = &tAdc1Dma;
...
{
	HAL_DMA_IRQHandler(&tAdc1Dma);
}

void TestAdcDMA(void)
{
	uint32_t uiRxData = 0;

	HAL_ADC_Start_DMA(&tAdc1, &uiRxData, sizeof(uiRxData));
}
